...
为此我们提出了m-K最近邻 (mKNN)指标(m-K-nearest neighbor)， 公式如式~(\ref{eq:mknn})所示，其中 $\bar x_b, k, N_b, N, B$ 分别表示：批次为b的样本周围同类批次的样本平均个数，自主定义的最近邻取样数，批次为b的样本总数，所有样本的总数以及批次的种类数。利用mKNN指标，我们可以逐批次逐样本地衡量其批次效应的严重性，
...
%TODO 讨论deseq的基本原理，注意表达要修改


%TODO 展示基本的DE结果并简单讨论

~\ref{fig:lulab_de}，癌症类型分别为肝癌和早期肝癌。
(A) 每个基因的对数fold change和对数调整后p值的火山图; (B) 前十个差异表达基因的
fold change, 表达值以及对数调整后p值; (C) 挑选出的差异表达基因的热图以显示其对于癌症和正常样本
的分类效果。
\begin{figure}[H] % use float package if you want it here
    \centering
...
~\ref{fig:scirep_de}，癌症类型分别为结肠癌和早期结肠癌。
(A) 每个基因的对数fold change和对数调整后p值的火山图; (B) 前十个差异表达基因的
fold change, 表达值以及对数调整后p值; (C) 挑选出的差异表达基因的热图以显示其对于癌症和正常样本
的分类效果。
\begin{figure}[H] % use float package if you want it here
