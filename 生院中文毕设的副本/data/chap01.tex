...
\begin{table}[htb]
  ...
  \label{tab:template-files}
    \begin{tabularx}{\linewidth}{X|X|X|X}
      \toprule[1.5pt]
      {\heiti 文件名} & {\heiti 描述} & {\heiti 描述} & {\heiti 描述} \\\midrule[1pt]
      thuthesis.ins & \LaTeX{} 安装文件，\textsc{DocStrip}\footnote{表格中的脚注} & 1  & 1 \\
      thuthesis.dtx & 所有的一切都在这里面\footnote{再来一个}。& 1  & 1 \\
      thuthesis.cls & 模板类文件。& 1  & 1 \\
      thuthesis.cfg & 模板配置文。cls 和 cfg 由前两个文件生成。& 1  & 1 \\
      thuthesis-numeric.bst    & 参考文献 BIB\TeX\ 样式文件。& 1  & 1 \\
      thuthesis-author-year.bst    & 参考文献 BIB\TeX\ 样式文件。& 1  & 1 \\
      thuthesis.sty   & 常用的包和命令写在这里，减轻主文件的负担。& 1  & 1 \\
      \bottomrule[1.5pt]
    \end{tabularx}
  \end{minipage}
\end{table}
...
贝叶斯公式如式~(\ref{equ:chap1:bayess})，其中 $p(y|\mathbf{x})$ 为后验；
$p(\mathbf{x})$ 为先验；分母 $p(\mathbf{x})$ 为归一化因子。
\begin{equation}
\label{equ:chap1:bayess}
p(y|\mathbf{x}) = \frac{p(\mathbf{x},y)}{p(\mathbf{x})}=
\frac{p(\mathbf{x}|y)p(y)}{p(\mathbf{x})} 
\end{equation}
